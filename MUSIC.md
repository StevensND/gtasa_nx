# GTA: SA NX - Restoring The Cut Songs from GTA SA Vita

### Table of Contents
+ [GTA: SA NX - Restoring The Cut Songs](#gta-sa-nx---restoring-the-cut-songs)
...
- Finally, after creating the `.osw.idx` files for each one of them, just copy all of the `.osw` with their `.osw.idx` files to the `STREAMS` folder on your vita in `/switch/gtasa/audio/STREAMS` and overwrite if asked.