 ...
 *
 * Loads multiple modules with cross-module symbol resolution and supports
...
// RELR/ANDROID_RELR packed relative relocations: add load base to each location
        ...
        switch (type) {
          case R_AARCH64_ABS64:
            if (sym->st_shndx == SHN_UNDEF) {
              // imported ABS64 (e.g. RTTI type_info): resolve after all modules load
              *ptr = rels[j].r_addend;
              deferred_abs64++;
            } else {
...
// look up a defined symbol in one module; returns its runtime address or 0
...
static uintptr_t so_resolve_symbol(so_module *mod, DynLibFunction *funcs, int num_funcs, const char *name) {
  // Import table wins over module exports so the donor's OpenAL symbols bind
  // to native openal-soft; only libc++/RTTI symbols fall through to the donor.
            ...
            if (sym->st_shndx == SHN_UNDEF) {
              ...
              if (addr) {
                *ptr = addr + rels[j].r_addend;
                ...
                // point it somewhere that faults obviously if ever called
